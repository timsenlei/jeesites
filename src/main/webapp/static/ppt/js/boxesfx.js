;( function( window ) {
	...
	BoxesFx.prototype._init = function() {
		...
		this.nav.innerHTML = '<span  id="prev"  onclick="prePage()"  class="prev"><i></i></span><span  id="next" onclick="nextPage()"  class="next"><i></i></span>';
	...
	BoxesFx.prototype._navigate = function( dir ) {
		...
		if(dir == 'home'  && this.current == 0) {
			$("#alertbox").show()
			return
		}
		if(dir == 'prev'  && this.current == 0) {
			$("#alertbox").show()
		...
		if(dir == 'next' && this.current+1 == this.panelsCount) {
			$("#alertbox1").show()
			if(!$("#alertbox1").is(":hidden")){
				setTimeout(function() {
					$("#alertbox1").hide("slow")
				}, 1500)
			}	
		...
		var cntTransTotal = 0, animatime = this.animatime,
